-- AOC - Pengingat rental 28 jam / hari + reminder H-4 jam + denda otomatis 10%
...
set rental_due_at = (
  ((o.rental_start::timestamp at time zone 'Asia/Jakarta')
   + (greatest(coalesce(o.rental_days, (o.rental_end-o.rental_start)+1, 1),1) * interval '28 hours'))
    ...
    late_fee_percent = case when coalesce(o.late_fee,0) > 0 then 10 else coalesce(o.late_fee_percent,0) end
...
begin
  ...
  loop
    ...
    -- Begitu lewat batas: denda otomatis satu kali = 10% dari seluruh harga sewa.
    if v_now >= v_order.rental_due_at
      ...
      v_fee := round(greatest(v_base,0) * 0.10, 2);
      ...
      if v_order.user_id is not null then
        ...
        values(
          ...
          'Pesanan '||v_order.order_number||' telah melewati batas pengembalian. Denda otomatis 10% dari seluruh harga sewa: Rp'||to_char(v_fee,'FM999G999G999G999D00'),
          v_now
        ) on conflict do nothing;
      end if;
    end if;
