import { supabase, rupiah, esc, message, addCart, getCart, updateCartBadge, cartStorageGet, cartStorageSet } from "./app.js";

let catalogLocations = [];
let selectedCatalogLocationId = cartStorageGet("aoc_location_id") || "";
...
async function loadCatalogLocations() {
  ...
  if (!catalogLocations.some(x => String(x.id) === String(selectedCatalogLocationId))) {
    ...
    if (selectedCatalogLocationId) cartStorageSet("aoc_location_id", selectedCatalogLocationId);
  }
}
...
async function setCatalogLocation(locationId) {
  ...
  cartStorageSet("aoc_location_id", id);
...
function renderLocationPicker() {
  ...
  if (selected && !selectedCatalogLocationId) {
    ...
    cartStorageSet("aoc_location_id", selectedCatalogLocationId);
  }
      ...
      <div class="body">
        ...
        ${item.type === "product" ? `
          <div class="catalog-quick-order">
            ...
            <button type="button" class="btn catalog-add-btn" data-catalog-add ${unavailable || maxQuantity <= 0 ? "disabled" : ""}>
...
function bindCatalogEvents(items, grid, mode) {
  ...
  grid.addEventListener("click", (event) => {
    ...
    try {
      ...
      console.error("AOC cart add failed:", error);
      const note = card.querySelector(".catalog-added-note");
      if (note) {
        note.textContent = "Gagal menambahkan ke keranjang. Silakan coba lagi.";
        note.classList.add("show");
      }
    }
  });
